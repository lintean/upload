			...
			<el-main :style="{minHeight: screenHeight - 120 + 'px'}" class="init">
				<div class="file_cnt" :style="{minHeight: screenHeight - 230 + 'px'}">
					<div style="height: 50px; line-height: 50px;">
						...
						<el-button @click="download" style="float: right; margin-top: 8px; margin-right: 20px;" type="primary" plain>下载文件</el-button>
					</div>
					...
					<div>
						<!-- 预览模块 除了图片都有问题-->
						<!-- 						<show-pdf v-if="fileMeta.type == 'doc'" :src="url" :page-count="count" 
						style="height:auto; width: 100%; margin: 0 auto;"></show-pdf> -->
						<!-- <Video v-if="fileMeta.type == 'video' || fileMeta.type == 'audio '" :src="url" :state="state" style="height:auto; width: 100%;"></Video> -->
						<el-image v-if="fileMeta.type == 'image'" :src="url"></el-image>
						<div v-if="fileMeta.type != 'image'" style="margin: 150px auto 0 auto;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getFileMeta() {
				let _this = this;
				Api.getFileMeta(this.fileId).then(
					res => {
						if (res.data.status === 200) {
			...
			download() {
				...
				Download.download(this.url, this.fileMeta.title +
					'.' + this.fileMeta.ext, () => {
						notify.close();
						setTimeout(function() {
							_this.$notify({
