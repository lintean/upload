	...
	<div style="padding: 24px;" @click="stopEditName">
		<div style="height: 50px;">
			...
			<div style="float: right;">
				<el-autocomplete :fetch-suggestions="getKeyword" @select="turnToSearch" @keyup.native.enter.stop="turnToSearchByKeyword"
				 v-model="keyword" placeholder="搜索你的资源">
		...
		<div>
			<el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_route">
				<el-breadcrumb-item v-for="(document, index) in path" :key="index" @click.native.stop="turnToPast(index)">{{document.title}}</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		<div class="light_divider"></div>
		<div style="display: flex; justify-content: flex-start; margin: 10px 0; flex-wrap: wrap;">
			<resource-card v-for="(doc, index) in docList" :key="index" shadow="always" @click.native.stop="clickReady && itemClicked(index)"
			 @dblclick.native.stop="clickReady && itemDBClicked(index)" class="document_card" :class="{'item_clicked': currentItemClicked == index, 'dc': !doc.isEditStatus}"
			 :doc="doc" :index="index">
			</resource-card>
		</div>
		...
		methods: {
			...
			itemDBClicked(index) {
				if (index > 0 && this.isCurrentFileLayout) {
					window.open("http://" + window.location.host + window.location.pathname + "singleFile.html#/?fileId=" +
						this.docList[index].id, "_sdfsf");
					return;
				};
			...
			refreshResource() {
				let _this = this;
				Api.getResources(this.currentResource.type, this.currentResource.id).then(
					res => {
			...
			backToUpper() {
				let _this = this;
				Api.getResources(this.docList[this.currentItemClicked].type, this.docList[this.currentItemClicked].id).then(
					res => {
						if (res.data.status === 200) {
							if (res.data.data == null) res.data.data = [];
							// 更新当前目录状态
				...
				Api.getResources(this.docList[this.currentItemClicked].type, this.docList[this.currentItemClicked].id).then(
					res => {
						if (res.data.status === 200) {
							...
							// 更新当前目录状态
							_this.isCurrentFileLayout = (_this.docList[_this.currentItemClicked].type == "doc");
							_this.currentResource = this.docList[this.currentItemClicked];
			...
			handleResource(res) {
				for (let i = 0; i < res.data.data.length; ++i) {
					if (res.data.data[i].thumbnail == "./assets/images/docCnt.png") res.data.data[i].thumbnail_url = require(
						"../assets/images/docCnt.png");
					if (res.data.data[i].thumbnail == "./assets/images/doc.png") res.data.data[i].thumbnail_url = require(
						"../assets/images/doc.png");
					if (res.data.data[i].thumbnail_url == null) res.data.data[i].thumbnail_url = require(
				...
				Api.Download(this.docList[this.currentItemClicked].id).then(
					res => {
						if (res.data.status === 200) {
							...
							Download.download(res.data.data.url, _this.docList[_this.currentItemClicked].title +
			...
			newResource(type) {
				let _this = this;
				Api.newResource(type, this.currentResource.id).then(
					res => {
						if (res.data.status === 200) {
			...
			resourceDelete() {
				...
				this.$alert('此操作将永久删除该资源, 是否继续?', '删除资源', {
					...
					callback: (action, instance) => {
						Api.deleteResource(_this.docList[_this.currentItemClicked].type, _this.docList[_this.currentItemClicked].id).then(
							res => {
								if (res.data.status === 200) {
